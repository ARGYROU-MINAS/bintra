let tocRoot; let tocBranch; let tocSel; let tocLoading; let tocLastID; let tableSel = null; let tabHead; let data; let tocTab; let tocInitRoot; let tocMemToc;
let rFStatus; let oldData; let tableSelTable;
let prevPage = '';
let initI;
...
function browserCheck () {
  ...
  if (browser.indexOf('msie') != -1) {
    ...
    if (browser.substring(browser.indexOf('msie') + 5, browser.indexOf('.', browser.indexOf('msie'))) > 5) { return ('ie6'); } else { return ('ie_old'); }
  } else if (browser.indexOf('firefox') != -1) { return ('ff'); } else if (browser.indexOf('opera') != -1) { return ('op'); } else if (browser.indexOf('gecko') != -1) { return ('ff'); }
}
...
function changeCSS (theClass, element, value, target) {
  ...
  for (let S = 0; S < target.length; S++) {
    for (let R = 0; R < target[S][cssRules].length; R++) {
...
function initItem (item) {
  if (prevPage != cont.document.location.href) { tableSel = null; }
...
function initLoad (src, toc, home) {
  ...
  if (qs.substring(0, qs.indexOf('=')) == 'goto') {
    const gotoPage = qs.substring(qs.indexOf('=') + 1).split(':');
    const fExt = home.substring(home.lastIndexOf('.'));
    var home = './EARoot/';

    for (let i = 0; i < gotoPage.length; i++) {
      home += 'EA' + gotoPage[i];

      if (i != gotoPage.length - 1) { home += '/'; } else { home += fExt; }
    }
  }
  ...
  if (qs.substring(0, qs.indexOf('=')) == 'guid') {
    ...
    let sGuid = new String(Linkguid);

    if (sGuid.substring(0, 1) == '{' && sGuid.substring(sGuid.length, sGuid.length - 1) == '}') { sGuid = sGuid.substring(1, sGuid.length - 1); }
    Linkguid = sGuid;
...
function LoadGuidMap (ResponseFunc) {
  ...
  let FirstTwoHexDigits = new String(Linkguid);
  FirstTwoHexDigits = FirstTwoHexDigits.slice(0, 2).toLowerCase();
  const src = 'js/data/guidmaps/' + 'GuidMap' + FirstTwoHexDigits + '.xml';
...
function OnReadyLoadGuidMap () {
  if (data.readyState == 4) {
    try {
      let Home = GetPageAddressFromMapString(data.responseText);
...
function GetPageAddressFromMapString (MapString) {
  let LG = new String(Linkguid);
  let Page = new String(MapString);

  LG = LG.toLowerCase(); // Make searching case insensitive.
  Page = Page.toLowerCase();

  let Pos1 = Page.search(LG); // Search the the guid in the string and return the position.
  Page = Page.slice(Pos1); // Remove the top half of the string.

  const Pos2 = Page.search('/'); // Search for the start of the web address.
  const Pos3 = Page.search(';'); // Search for the end of the web address.
...
function OnReadyGuidLink () {
  if (data.readyState == 4) {
    try {
      ...
      let Page = GetPageAddressFromMapString(data.responseText);
...
function initPage (src) {
  ...
  let curPage = cont.document.location + '';

  if (curPage.indexOf('EARoot') != -1) { curPage = curPage.substring(curPage.indexOf('EARoot')); } else { curPage = curPage.substring(curPage.lastIndexOf('/') + 1); }
  ...
  for (var j = 0; j < cont.document.getElementsByTagName('div').length; j++) {
    if (cont.document.getElementsByTagName('div')[j].className == 'ObjectDetailsNotes') {
      let tmpStr = cont.document.getElementsByTagName('div')[j].innerHTML;
  ...
  if (toc.document.body == null) {
    ...
    let parTree = curPage.substr(0, curPage.lastIndexOf('/')).substr(curPage.indexOf('/') + 1).replace(/EA/g, '').replace(/\//g, '.');
    if (parTree) {
      ...
      for (var j = 0; parTree.length > j; j++) {
        if (j >= 1) { tocTmp += '.' + parTree[j]; } else { tocTmp = parTree[j]; }
        ...
        const tmpCurToc = toc.document.getElementById('toc' + tocTmp).parentNode.childNodes;
...
function initPreLoad (content) {
  const preload = document.getElementById('IndexHeader').parentNode.insertBefore(document.createElement('div'),
  ...
  if (browser == 'ie6' || browser == 'ie7') { changeCSS('#Preload', 'height', top.document.body.clientHeight - 67 + 'px', 'this'); } else { changeCSS('#Preload', 'height', top.innerHeight - 67 + 'px', 'this'); }

  for (let i = 0; i < preloadIcons.length; i++) {
    icon[preloadIcons[i]] = new Image();
    icon[preloadIcons[i]].src = 'images/' + preloadIcons[i] + '.png';
  }

  const preloader = document.body.appendChild(document.createElement('div'));
...
function resizeFrames (str) {
  const resizeFrames = document.getElementById('resizeFrames');
  if (str == 'init') {
...
function resizeFramesOff (e) {
  ...
  const resizeFrames = document.getElementById('resizeFrames');
  if (document.getElementById('tmpRF') != null) { document.getElementById('tmpRF').parentNode.removeChild(document.getElementById('tmpRF')); }
...
function resizeFramesMove (e) {
  if (rFStatus != 0) {
    if (rFStatus == 2) { var posX = e + 2; } else if (browser == 'ff') { var posX = e.pageX; } else { var posX = event.x; }

    if (posX <= 150) { posX = 150; } else if (posX >= top.document.body.clientWidth / 2) { posX = top.document.body.clientWidth / 2; }
    ...
    if (document.getElementById('tmpRF') != null) { document.getElementById('tmpRF').style.left = (posX - 2) + 'px'; }

    if (browser == 'ie6') { document.getElementById('tocIFrame').style.width = (posX - 4) + 'px'; } else { document.getElementById('tocIFrame').style.width = (posX - 3) + 'px'; }
    ...
    if (tableSelTable != null) {
      if (tableSelTable.childNodes.length > 1 && document.getElementById('tmpRF') != null) {
        ...
        if (tableSelTable.childNodes[0].style) { tableSelTable.childNodes[0].style.width = (tableSelTable.offsetWidth - 19) + 'px'; }

        if (tableSelTable.childNodes[1].style) { tableSelTable.childNodes[1].style.width = (tableSelTable.offsetWidth - 19) + 'px'; }
      }
    }
  }
}
...
function tocBuild (data) {
  const tocType = new Array();
  const tocTypes = new Array();
  const tocErrs = new Array();
  ...
  for (var i = 0; i < tocTypes.length; i++) {
    for (let j = 0; j < tocType[tocTypes[i]].length; j++) {
      let build = tocBuildItem(tocType[tocTypes[i]][j]);
  ...
  while (tocErrs.length != 0) {
    ...
    const tmp = tocBuildItem(tocErrs.shift());
  ...
  if (tocLastID != 0) {
    const tocLastSrc = document.getElementById('toc' +
...
function tocBuildItem (i) {
  ...
  if (tocTab[i][0].indexOf(':') != -1) { tocBranch = document.getElementById('toc' + tocTab[i][0].substring(0, tocTab[i][0].lastIndexOf(':'))); } else if (tocTab[i][0].indexOf('.') != -1) { tocBranch = document.getElementById('toc' + tocTab[i][0].substring(0, tocTab[i][0].lastIndexOf('.'))); } else if (tocTab[i][0] != '0') { tocBranch = document.getElementById('toc0'); } else if (tocTab[i][1] == 3) { tocBranch = document.getElementById('System').parentNode.lastChild; } else { tocBranch = tocRoot; }
  ...
  if (tocTab[i][0] != 0 || tocTab[i][1] == 3) {
    ...
    if (tocTab[i][0].indexOf('.') == -1 && (tocTab[i][0].indexOf(':') == -1 || tocTab[i][0].indexOf('0') == 0)) { tocImage.src = 'images/join01.gif'; } else if (tocTab[i][0].indexOf('.') != -1 || tocTab[i][0].indexOf(':') != -1) {
      var tocTmp = tocTab[i][0].replace(':', '.');

      for (let m = 0; tocTmp.split('.').length > m + 1; m++) {
          ...
          tocTmp.lastIndexOf('.'))).parentNode.childNodes[m].src.substring(document.getElementById('toc' +
					...
					tocTmp.substring(0,
					  tocTmp.lastIndexOf('.'))).parentNode.childNodes[m].src.lastIndexOf('images')).indexOf('04') !=
						-1) { tocImage.src = 'images/join04.gif'; } else { tocImage.src = 'images/join03.gif'; }
        tocImage = tocBranch.appendChild(document.createElement('img'));
      }
  ...
  if (top.icon[tocTab[i][4]]) { tocImg.src = top.icon[tocTab[i][4]].src; } else { tocImg.src = 'images/' + tocTab[i][4] + '.png'; }

  if (tocTab[i][3]) { tocImg.id = tocTab[i][3]; }

  // Set Text/Link
  if (tocTab[i][5] && tocTab[i][8] != '0Diagram') { tocBranch.lastChild.id = tocTab[i][5]; } else { tocBranch.lastChild.id = '0'; }
  ...
  if (tocTab[i][3]) {
    ...
    if (tocTab[i][9]) { tocText.id = tocTab[i][9]; } else if (tocTab[i][7]) { tocText.id = tocTab[i][7]; }
...
function toggleDiv (idDiv, idImage) {
  const ele = cont.document.getElementById(idDiv);
...
function toggleData (src) {
  const dataRoot = cont.document.getElementById(src).parentNode.parentNode.parentNode;
  ...
  for (let i = 0; i < dataRoot.getElementsByTagName('tr').length; i++) {
    if (dataRoot.getElementsByTagName('tr')[i].id.indexOf(src) !=
      ...
      cont.document.getElementById(src).src.lastIndexOf('images')).indexOf('minus') !=
				-1 && dataRoot.getElementsByTagName('tr')[i].getElementsByTagName('td')[1].firstChild != null) { dataRoot.getElementsByTagName('tr')[i].style.display = ''; } else if (dataRoot.getElementsByTagName('tr')[i].id.indexOf(src) != -1) { dataRoot.getElementsByTagName('tr')[i].style.display = 'none'; }
  }
...
function toggleItem (item, type) {
  if (tableSel != null) {
    const obj = cont.document.getElementById(tableSel + 'Table');
  ...
  if (tableSelTable.id == 'LinkedDocumentTable') { tableSelTable.style.overflow = 'scroll'; }
  ...
  if (cont.document.body.offsetHeight - contWHeight > 0) {
    ...
    if (tableSelTable.getElementsByTagName('table').length != 0) {
      ...
      if (browser == 'ff' || browser == 'op') { tabHead.style.width = tableSelTable.offsetWidth + 'px'; }
      tabHead.style.top = tableSelTable.offsetTop + 1 + 'px';

      for (let i = 0; i < tabHead.getElementsByTagName('tr').length; i++) {
